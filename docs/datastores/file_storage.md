...
The first datastore is a flat-file database of JSON files on disk. This can be used by specifying `file://` for the `--cache` option in the command line interface, like so:

```
$ transload get metadata \
    --provider environment_canada \
    --station_id CXCM \
    --cache file:///Volumes/ramdisk/datastore/weather
```
