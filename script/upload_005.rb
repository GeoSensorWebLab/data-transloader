...
class TestStation < Transloader::KLRSHistoricalEnergyStation
  ...
  def upload_observations(destination, interval, options = {})
    ...
    @times[:upload] = Benchmark.measure do
      progress = ProgressBar.create(total: observations.length, format: "%c/%u %a |%W| %e")

      MQTT::Client.connect(host: $mqtt_url, port: 1883, client_id: "howdy", ack_timeout: 60) do |c|
        responses = observations.collect do |observation|
          ...
          if datastream.nil?
            ...
            datastream_path = datastream_url.split("/").last
            c.publish("v1.0/#{datastream_path}/Observations", observation.to_json, false, 1)
            progress.increment
          end
        end
      end # MQTT
